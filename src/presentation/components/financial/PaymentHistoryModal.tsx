    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div className="bg-obsidian-900/90 border border-obsidian-850 rounded-2xl w-full max-w-lg shadow-2xl animate-scale-up">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-obsidian-850 bg-obsidian-950 rounded-t-2xl">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-205 p-1 transition-colors"
            type="button"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="px-6 py-2 border-b border-obsidian-850 bg-obsidian-950/40 flex flex-wrap items-center justify-between gap-3">
          <div className="flex gap-2">
            <button
              ...
              className={`text-[10px] uppercase font-black tracking-wider px-3 py-1.5 rounded transition-all ${historyViewTab === 'all' ? 'bg-slate-100/5 text-slate-205 border border-slate-200/10' : 'text-slate-500 hover:text-slate-300'}`}
            >
              Todos os pagamentos
            </button>
            <button
              ...
              className={`text-[10px] uppercase font-black tracking-wider px-3 py-1.5 rounded transition-all ${historyViewTab === 'by_month' ? 'bg-slate-100/5 text-slate-205 border border-slate-200/10' : 'text-slate-500 hover:text-slate-300'}`}
            >
              Ver por mês
            </button>
          </div>
            ...
            filteredPays.map((pay) => (
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <span className="font-bold text-xs text-slate-202 font-mono">
                    {pay.mesRef}
                  </span>
