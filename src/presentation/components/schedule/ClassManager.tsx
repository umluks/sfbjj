...
import type { Turma } from '@/domain/models/class';

interface ClassManagerProps {
  turmas: Turma[];
  setTurmas: React.Dispatch<React.SetStateAction<Turma[]>>;
  onCreate: (turma: Omit<Turma, 'id'>) => Promise<Turma>;
  onUpdate: (id: number, turma: Partial<Turma>) => Promise<void>;
  onDelete: (id: number) => Promise<void>;
}

const CATEGORIES = ['Adulto', 'Kids', 'Open Match'] as const;
...
export const ClassManager: React.FC<ClassManagerProps> = ({ 
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await onUpdate(editingId, { nome, categoria });
      } else {
        await onCreate({ nome, categoria });
      }
      setShowModal(false);
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Cadastre e edite as turmas da academia diretamente no Supabase.
          </p>
        </div>
        <button
          onClick={() => openModal()}
