...
export const SchedulePage: React.FC<SchedulePageProps> = ({ loggedUser }) => {
  const [activeTab, setActiveTab] = useState<'grade' | 'turmas' | 'horarios'>('grade');
    ...
    <div className="space-y-6">
      ...
      <div className="mt-6">
        ...
        {activeTab === 'turmas' && isAdmin && (
          <ClassManager turmas={turmas} setTurmas={setTurmas} onCreate={createTurma} onUpdate={updateTurma} onDelete={deleteTurma} />
        )}
        
        {activeTab === 'horarios' && isAdmin && (
          <TimeSlotManager schedule={classes} setSchedule={setClasses} turmas={turmas} professores={professores} onCreate={createClass} onUpdate={updateClass} onDelete={deleteClass} />
        )}
      </div>
    </div>
  );
