...
export const StudentProfilePage: React.FC<StudentProfilePageProps> = ({ alunoId, initialSubTab, hideSidebarMenu }) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fade-in pb-12 text-left w-full max-w-full min-w-0 overflow-x-hidden">
      {/* Header Titular */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-100 tracking-tight flex items-center gap-3">
            <User className="w-7 h-7 sm:w-8 sm:h-8 text-gold-500 shrink-0" />
      ...
      {student && !isEditingAdmin && !isEditingTeacher && (
        <div className="bg-gradient-to-br from-obsidian-900 via-obsidian-900 to-obsidian-950 border border-obsidian-800 rounded-3xl p-5 sm:p-7 shadow-2xl relative overflow-hidden w-full max-w-full min-w-0">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10 w-full min-w-0">
            ...
            {/* Pílula de Peso e Categoria IBJJF */}
      ...
      {/* Sub-Abas de Navegação Responsivas */}
      {!hideSidebarMenu && (
        <div className="w-full max-w-full overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-2 p-1.5 bg-obsidian-950 border border-obsidian-850 rounded-2xl shadow-inner min-w-max">
            <button
      ...
      {/* Conteúdo Central */}
      <div className="bg-obsidian-900/60 border border-obsidian-850 p-4 sm:p-6 md:p-8 rounded-3xl shadow-xl w-full max-w-full min-w-0 overflow-x-hidden">
        {activeSubTab === 'profile' && (
          <PersonalInfoForm
