...
export const ScheduleGrid: React.FC<ScheduleGridProps> = ({ loggedUser }) => {
  ...
  return (
    <div className="space-y-6">
      {isAdmin && (
        <div className="flex bg-obsidian-850 p-1 rounded-xl border border-obsidian-800 w-full max-w-2xl overflow-x-auto">
          <button
            onClick={() => setActiveTab('grade')}
            className={`flex-1 flex justify-center items-center gap-2 py-2.5 px-4 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${
              activeTab === 'grade'
